...
async function loadCatastroData(): Promise<CatastroProperty[]> {
  try {
    console.log("Fetching cadastral data from API...");
    const response = await fetch('/api/catastro/data');
    
    if (!response.ok) {
      console.error("API response not OK:", response.status, response.statusText);
      throw new Error(`No se pudo cargar el archivo del catastro: ${response.status} ${response.statusText}`);
    }
    ...
    if (!responseData.success) {
      console.error("API response indicated failure:", responseData.message);
      throw new Error(responseData.message || 'Error desconocido al cargar datos');
    }
    
    console.log(`API returned ${responseData.data.length} records successfully`);
...
export default function CatarrojaPage() {
  ...
  useEffect(() => {
    async function fetchCatastroData() {
      setLoading(true);
      try {
        console.log("Starting to fetch cadastral data...");
        const catastroRecords = await loadCatastroData();
        
        if (catastroRecords.length === 0) {
          console.warn("No cadastral records returned from API");
          toast.error("No se encontraron registros del catastro");
        ...
        console.log("Converting to Address format...");
        ...
        toast.error("No se pudieron cargar los datos del catastro: " + (error as Error).message);
  ...
  const currentItems = useMemo(() => {
    return filteredAddresses.slice(indexOfFirstItem, indexOfLastItem);
  }, [filteredAddresses, indexOfFirstItem, indexOfLastItem]);
  
  // Reset pagination when filters change
  useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm, showCatastro, isZoneSelected]);
            ...
            {loading ? (
              ...
              <>
                <div className="p-2 bg-gray-50 border-b border-gray-200 text-xs text-gray-500">
                  Mostrando {currentItems.length} de {filteredAddresses.length} resultados (página {currentPage} de {Math.ceil(filteredAddresses.length / itemsPerPage)})
                </div>
                ...
                {filteredAddresses.length > itemsPerPage && (
                  <div className="p-2 border-t border-gray-200 flex justify-between items-center bg-white">
                    ...
                    <span className="text-sm text-gray-500">
                      Página {currentPage} de {Math.ceil(filteredAddresses.length / itemsPerPage)}
                    </span>
                    <button
                      className="px-2 py-1 text-sm text-gray-700 rounded hover:bg-gray-100 disabled:opacity-50"
