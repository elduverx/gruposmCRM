...
function formatStreetName(streetName: string): string {
  ...
  if (lowerStreet.includes('pz mercat del') || lowerStreet.includes('plaza mercat del')) {
    return 'Plaça del Mercat';
  }
  
  if (lowerStreet.includes('musico j manuel izquierdo') || lowerStreet.includes('músico j manuel izquierdo')) {
    return 'Calle del Músico José Manuel Izquierdo';
  }
  ...
  for (const [abbr, full] of Object.entries(abbreviations)) {
    if (lowerStreet.startsWith(abbr + ' ') || lowerStreet.startsWith(abbr + '.')) {
      const restOfStreet = streetName.substring(abbr.length).trim();
      // Si la calle comienza con "del" o "de la", mantener estas palabras
      if (restOfStreet.toLowerCase().startsWith('del ') || restOfStreet.toLowerCase().startsWith('de la ')) {
        return full + ' ' + restOfStreet;
      }
      return full + ' ' + restOfStreet;
    }
  }
